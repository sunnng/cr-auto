...
- `internal/game/`: `taskbuilder.go` (standard task wrapper: 开关/就绪/让渡/离开广场) + `register.go` (M1 skeleton; M2a 注入网络联机状态不稳定守卫 + 矿山模块). `kingdom/`/`popup/` 通用页（特征库+页面）；`mine/` 矿山特征库与首页；`mine/route/` 王国↔矿山路由（涉及勘查/开采页的路由因 Go 包循环限制随使用方存放）；`mine/{survey,mining,battle,jelly}/` 各矿山模块（页面/会话/任务，均桌面可测）。
